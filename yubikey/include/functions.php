...
function yubikey_getModuleAdminLink($moduleName='yubikey') {
	global $icmsModule;
...
*/
function yubikey_getLinkedUnameFromId($userid, $name = false, $users = array (),
		$withContact = false)
{
	...
	if($userid > 0)
	{
		...
		if(is_object($user))
		{
			...
			if(!empty ($fullname)) {
				$linkeduser = "$fullname [<a href='" . ICMS_URL	. "/userinfo.php?uid=" . $userid
